...
class ContestSubmissionAPIView(APIView):
    ...
    def post(self, request):
        ...
        if request.user.admin_type != SUPER_ADMIN and datetime.datetime.now().hour <= 6 :
            return error_response("时间晚了，去休息吧\n")
        if serializer.is_valid():
            data = serializer.data
    ...
    def get(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                if submission.contest_id:
                    contest = Contest.objects.get(id=submission.contest_id)
                    problem = ContestProblem.objects.get(contest=contest, id=submission.problem_id)
            except Problem.DoesNotExist:
                return error_response(u"题目不存在")
            try:
                _judge.delay(submission.id, problem.time_limit, problem.memory_limit, problem.test_case_id,
